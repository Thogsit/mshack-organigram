...
export class Db {
  private static content: DbContent = Db.convertJsonToContent(
    JSON.parse(fs.readFileSync("resources/initial_graph.json", "utf-8")),
  );
  ...
  public static getAsGraphData(): GraphData {
    return {
      nodes: this.content.organizations.map((node) =>
        GraphNode.fromOrgNode(node),
      ),
      combos: this.content.groups.map((group) =>
        GraphCombo.fromOrgGroup(group),
      ),
